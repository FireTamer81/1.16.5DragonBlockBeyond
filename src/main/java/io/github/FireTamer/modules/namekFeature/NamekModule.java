package io.github.FireTamer.modules.namekFeature;
...
import io.github.FireTamer.common.FluidValues;
import io.github.FireTamer.commonInit.BlockInit;
import io.github.FireTamer.modules.namekFeature.blocks.*;
import io.github.FireTamer.modules.namekFeature.codedFeatures.NamekTree;
...
@Module(id = @RL(modid = DBB_Main.MOD_ID, path = "namek_feature"))
public class NamekModule extends ModuleHelper implements IModule {
    ...
    public static final Block NAMEK_KELP_TOP = new NamekKelpTop(AbstractBlock.Properties.of(Material.REPLACEABLE_WATER_PLANT)
            .noCollission()
            .randomTicks()
            .instabreak()
    ...
    public static final Block NAMEK_KELP_STEM = new NamekKelpStem(AbstractBlock.Properties.of(Material.REPLACEABLE_WATER_PLANT)
            .noCollission()
            .instabreak()
            .randomTicks()
            .sound(SoundType.WET_GRASS));
    ...
    public static final Block NAMEK_TREE_SAPLING = new SaplingBlock(new NamekTree(), AbstractBlock.Properties.of(Material.PLANT)
            .noCollission()
            .randomTicks()
            .instabreak()
    ...
    public static final Block NAMEK_SEAGRASS = new NamekSeaGrass(AbstractBlock.Properties.of(Material.REPLACEABLE_WATER_PLANT)
            .noCollission()
            .instabreak()
            .sound(SoundType.WET_GRASS)
    );
    ...
    public static final Block NAMEK_TALL_SEAGRASS = new NamekSeaGrassTall(AbstractBlock.Properties.of(Material.REPLACEABLE_WATER_PLANT)
            .noCollission()
            .instabreak()
            .sound(SoundType.WET_GRASS)
    );
    ...
    public static final Block NAMEK_LEAVES = new LeavesBlock(AbstractBlock.Properties.of(Material.LEAVES)
            .strength(0.2F)
            .sound(SoundType.GRASS)
            .randomTicks()
    ...
    public static final Block AJISA_BUSH = new AjisaBush(AbstractBlock.Properties.of(Material.LEAVES)
            .strength(0.2f)
            .sound(SoundType.GRASS)
            .noOcclusion()
            .randomTicks());
    ...
    public static final BetterRegistryObject<FlowingFluid> NAMEK_FLUID_SOURCE = fluid(() -> new ForgeFlowingFluid.Source(Properties.NAMEK_FLUID_PROPERTIES));

    @RegisterFluid("namek_fluid_flowing")
    public static final BetterRegistryObject<FlowingFluid> NAMEK_FLUID_FLOWING = fluid(() -> new ForgeFlowingFluid.Flowing(Properties.NAMEK_FLUID_PROPERTIES));

    @RegisterBlock("namek_fluid_block")
    public static final FlowingFluidBlock NAMEK_FLUID_BLOCK = new FlowingFluidBlock(NAMEK_FLUID_SOURCE::get, AbstractBlock.Properties.copy(Blocks.WATER));



    public static class Properties {
        public static final ForgeFlowingFluid.Properties NAMEK_FLUID_PROPERTIES = registerFluid(FluidValues.NAMEK_WATER, NAMEK_FLUID_SOURCE::get, NAMEK_FLUID_FLOWING::get, NAMEK_WATER_BUCKET, NAMEK_FLUID_BLOCK);


        private static ForgeFlowingFluid.Properties registerFluid(FluidValues value, Supplier<FlowingFluid> still, Supplier<FlowingFluid> flowing, Item bucket, FlowingFluidBlock block) {
            return new ForgeFlowingFluid.Properties(still, flowing, FluidAttributes.builder(STILL_RL, FLOWING_RL)
                    .color(value.getColor())
                    .density(Math.round(value.getDensity()))
                    .viscosity(Math.round(value.getDensity()))
                    .temperature(Math.round(value.getTemperature()))
    ...
    public static void greenFogInNamekWater(EntityViewRenderEvent.FogColors event)
    {
        ...
        if (fluidstate.getType() == NamekModule.NAMEK_FLUID_FLOWING.get() || fluidstate.getType() == NamekModule.NAMEK_FLUID_SOURCE.get())
        {
            event.setBlue(0.09F);
    ...
    //The only weird thing about this event I have done is that you cannot just click any side of the Namek Dirt/Grass and it function,
    ...
    //This is being used to recreate the vanilla underwater grow effect, but for my Namek Fluid and Sea Grasses
    //Remember to check "BonemealEvent" from forge and "BoneMealItem" form minecraft to get this working.
    @SubscribeEvent
    public static void namekFluidBonemealUse(BonemealEvent event) {
        ...
        if (fluidstate.getType() == NamekModule.NAMEK_FLUID_FLOWING.get() || fluidstate.getType() == NamekModule.NAMEK_FLUID_SOURCE.get()) {
            if (lookingAt != null && lookingAt.getType() == RayTraceResult.Type.BLOCK) {
                ...
                BlockPos blockpos_above_2 = blockpos_above_1.above();

                BlockState namekSeagrassState = NamekModule.NAMEK_SEAGRASS.defaultBlockState();
                BlockState namekTallSeagrassState = NamekModule.NAMEK_TALL_SEAGRASS.defaultBlockState();

                //player.displayClientMessage(new StringTextComponent("Hey, Bonemeal + Namek water + solid block shoudl do something"), false);
                //Here starts the more experimental stuff... Basically, don't delete anything above here.
                Task:
                for (int i = 0; i < 128; ++i) {
